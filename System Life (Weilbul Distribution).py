...
for i in range(1, N+1):
    ...
    # renewal
    cold_standby_2 = np.random.weibull(1, size=n)
    ...
    if np.array_equal(X1, first_fail):
        ...
        second_fail = np.minimum(X1, X3)
        if np.array_equal(second_fail, X1):
            t = (np.minimum(cold_standby, np.maximum(cold_standby_2, X3-X1)) + X1) + (np.minimum(cold_standby_2, np.maximum(cold_standby)))
        else:
            t = np.minimum(X1-X3, np.maximum(cold_standby, cold_standby_2)) + X3
    else:
        second_fail = np.minimum(X1, X2)
        if np.array_equal(second_fail, X1):
            t = (np.minimum(cold_standby, np.maximum(cold_standby_2, X2-X1)) + X1) + (np.minimum(cold_standby_2, np.maximum(cold_standby)))
        else:
